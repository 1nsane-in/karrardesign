...
function SvgImage({ svg, className, alt }) {
    const src = useMemo(() => `data:image/svg+xml;utf8,${encodeURIComponent(svg)}`, [svg]);
...
export function WorldMap({
    ...
    const svgMap = useMemo(() => {
        ...
        return map.getSVG({ radius: 0.22, color: "#00000040", shape: "circle", backgroundColor: "transparent" });
    }, []);
            ...
            >
                {showRoutes &&
                    dots.map((dot, i) => {
                        ...
                        const initial = prefersReducedMotion ? { pathLength: 1 } : { pathLength: 0 };
                        const animate = prefersReducedMotion ? { pathLength: 1 } : { pathLength: 1 };
                        const transition = prefersReducedMotion ? { duration: 0 } : { duration: 1, delay: 0.3 * i, ease: "easeOut" };
                        return (
                            <g key={`path-group-${i}`}>
                                <motion.path
